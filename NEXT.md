...
- **The kitchen board duplicates itself on a reload, and it is not fixed.** Clearing the shared list and refilling it does not work: the removals are not visible to `peek()` before the inserts run. Second reload, every ticket twice. It shows as a picked up order keeping its card, and it reproduces by running `KitchenBoardLayoutBrowserlessTest` then `KitchenBoardMultiUserBrowserlessTest`. `mvn verify` does not hit it, which is luck. Two fixes made it worse and were reverted; both are described in the feedback row. This is the first thing to pick up.
