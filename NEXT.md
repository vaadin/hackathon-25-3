...
- **The browser tier has one flake.** Running all six `IT` classes together, a sign in occasionally lands back on `/login` and the test that follows asserts on an empty page. Re-running is green. It is the CSRF race `BrowserIT.signIn` already waits for, and the wait is evidently not sufficient under load.
