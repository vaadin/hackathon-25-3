...
**Tasks:**
- [ ] Parent route with `@RouteParent`, three child routes, each with `@DynamicPageTitle`
- [ ] `Breadcrumbs` in router driven mode, no hand written trail
- [ ] Step state in the session signal bean, surviving back, forward and refresh
- [ ] Steps ahead of the current one are not clickable, which ROUTER mode gives for nothing: the trail is the ancestors of the current route, so a step not yet reached is not in it
...
**Tasks:**
- [ ] `BeanValidationBinder` with `setValidationGroups(Default.class)` per step
- [ ] Review validates with `Default` plus `OnSubmit`
- [ ] Place order enabled from a computed signal over the per step validity signals
- [ ] Draft save path at the counter validates with `OnDraft` only
...
**Tasks:**
- [ ] One service call: deduplicate customer, snapshot prices, book slot, generate reference and token, first history entry
...
**Tasks:**
- [ ] `/track/{reference}` with the opaque token, identical not found for wrong reference and wrong token
- [ ] Timeline from history, items, slot
- [ ] Live state through the shared signal of epic 08, degrading to a manual refresh until that epic lands
- [ ] Cancel offered only while new, with confirmation
- [ ] Rate limit per IP
...
- [ ] Skip unavailable products and name them on the cart page
