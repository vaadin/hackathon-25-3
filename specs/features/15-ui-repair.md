...
- [ ] The login page is themed on a cold load, signed out, with no prior session
- [ ] Export as CSV downloads a file whose contents are the rows on screen
- [ ] The invoice print page renders with no application shell around it
- [ ] A product page's header is the product's name
...
Nothing in this document is fixed yet. Every defect above was reproduced in a browser before it was written down.
