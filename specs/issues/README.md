...
| `13-ai-tool-schema-dropped.md` | flow | `ai-live.zip`, route `/tools`, two tools and one turn. Needs a key |
...
| `27-ai-form-discovery-composite.md` | flow | `ai-live.zip`, route `/form`, a field inside a Composite. Needs a key |
| `28-ai-turn-with-no-trace.md` | flow | `ai-live.zip`, route `/grid`, two questions. Needs a key |
| `29-ai-source-tracking-never-arrives.md` | flow | `ai-live.zip`, route `/form`, one turn. Needs a key |
...
Ten findings died when somebody tried to reproduce them. Every one was killed by building the project or repeating the steps, not by rereading the row.
...
| `42` the AI field marker's badge does not open its popover | It opens. A real mouse click through the driver opens the popover, `aria-expanded` goes to `true`, and it says "This field value was modified by AI." with a Revert Value button. The original attempt used a synthesised click |
| A GridPro cell does not enter edit mode from a synthesised double click | True, and not a bug: `page.mouse.dblclick` on a price cell puts an editor in the cell, sets `editing`, and focuses the input. Two findings died to the same cause on the same day, and both were about a gesture nobody performed |
...
| `13-ai-tool-schema-dropped.md` | Was "a tool whose schema is not valid JSON is dropped silently". A live run offered both tools and called both, and the turn answered correctly. What is real is the diagnosis: the error is logged twice a turn, names Jackson and a redacted source, and never names the tool |
...
| [`ai-live.zip`](projects/ai-live.zip) | 8140 | Four routes over a live model: a broken tool schema, a form with a field inside a Composite, a grid over a TIME column, and a test that hangs without its timeout |
...
| --- | --- | --- |
| Drafted as an issue | 41 drafts, several of them bundling a family of rows | This directory, 27 of them with a project zip |
| Corrected by their own reproduction | 5 | Still real, saying something different: the table above |
| Real, not yet reduced | 2 | The not ready table, each with what it needs |
| Not a report, it was our own rule | 9 | The specifications: `08-testing.md`, `10-dev-loop.md`, `03-architecture.md` |
| Withdrawn or not reproducible | 10 | Deleted, with the lesson from each in the table above |
...
Six drafts need something the reader may not have, and each says so at the bottom: a Charts licence for `09`, and an OpenAI key for `13`, `27`, `28`, `29`, `41` and `46`. All of those have now been run against `gpt-4o-mini`, so what they claim is what the run printed, and `ai-live.zip` reproduces each in one route. `48` needs neither: its provider is fifteen lines and calls no model.
